...
- **Notification actions.** Daily reminders carry category `STREAK_REMINDER` with a "✓ Mark done" action (`opensAppToForeground: false`). Two delivery paths cover the two app states:
  - **Foreground / warm JS** — `<NotificationActionHandler />` is mounted at the root inside `AppDataProvider`; it subscribes to `addNotificationResponseReceivedListener` and also calls `getLastNotificationResponseAsync` once on mount. Calls `markComplete` from context, which fires the undo toast and confetti.
  - **Killed / background** — `src/utils/notificationTask.ts` calls `TaskManager.defineTask(NOTIF_RESPONSE_TASK, ...)` at module init (imported from `index.ts` *before* `App` so the OS can route to it on cold dispatch) and `Notifications.registerTaskAsync` is called from `configureNotifications`. The background task reads `streakapp_v1` directly, appends today's completion via `computeUpdatedStreak`, clears `freezeNotifIds`, cancels their scheduled notifications, and dismisses the source notification. Constants `ACTION_MARK_DONE` and `STREAK_CATEGORY_ID` live in `src/utils/notificationConstants.ts` to avoid a cycle between `reminders.ts` and `notificationTask.ts`. The two paths do not double-fire because Expo dispatches foreground responses to the JS listener and background responses to the registered task — never both. Background-task notification responses do **not** work in Expo Go; they require a dev client / EAS build.
  - Plain-tap (default action) is intentionally a no-op aside from opening the app — it does not auto-mark, so the user can review the heatmap first. Only the `MARK_DONE` action calls `markComplete`.
